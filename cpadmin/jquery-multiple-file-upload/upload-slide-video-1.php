			...
			$file_new_name = time().".".$file_ext;	//NEW File Name Defined
       	 	...
       	 	move_uploaded_file($_FILES["slide_video_file"]["tmp_name"],$output_dir. $file_new_name);
       	 	 //echo "<br> Error: ".$_FILES["slide_video_file"]["error"];
						...
						$date_added = time();
						$query = "INSERT INTO slideshow_media (Section_Id, Slide_Title, Slide_Description, Slide_Link, Slide_Link_Type, Slide_Src_File, Slide_Src_Type, Date_Added, Status, Sort_Id)VALUES('".$_REQUEST['section_id']."','".$_REQUEST['slide_title']."','".$_REQUEST['slide_descript']."', '".$_REQUEST['slide_link']."', '".$_REQUEST['slide_link_type']."', '".$file_new_name."', '".$_REQUEST['slide_type']."', '".$date_added."', '1', '".$sort_id."')";
						mysql_query($query);
