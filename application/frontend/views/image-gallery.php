...
<script type="text/javascript" src="<?php echo base_url();?>assets/js/dragscrollable.js"></script>
...
var gallery_slides = new Array();
var gallery_obj = {

<?php $counter = 0;?>
<?php $total_count = count($gallerydata);?>
<?php foreach( $gallerydata as $galleryslide ) : ?>
<?php /*?>gallery_slides["slide_src"][<?php echo $counter;?>] = "<?php echo base_url();?>/uploads/gallery/image-gallery/<?php echo $galleryslide["Slide_Src_File"]; ?>";
gallery_slides["slide_title"][<?php echo $counter;?>] = "<?php echo $galleryslide["Slide_Title"]; ?>";<?php */?>


    "<?php echo $counter;?>" : {
        "slide_src" : "<?php echo base_url();?>/uploads/gallery/image-gallery/<?php echo $galleryslide["Slide_Src_File"]; ?>",
        "slide_title" : "<?php echo $galleryslide["Slide_Title"]; ?>",
		"slide_description" : "<?php echo $galleryslide["Slide_Description"]; ?>",
		"photographer_name" : "<?php echo $galleryslide["Photographer_Name"]; ?>",
		"photographer_gps" : "<?php echo $galleryslide["Photographer_Gps"]; ?>"
    },
...
var gallery_slides_counter = 0;
var slide_pos_counter = 1;
var total_slides = <?php echo $total_count;?>


function prevSlide() {
  if(gallery_slides_counter == 0) {

  }else {
    	gallery_slides_counter -= 1;  
      	$('#main-img').attr('src', gallery_obj[gallery_slides_counter].slide_src);
	    $('#slide_title').text(gallery_obj[gallery_slides_counter].slide_title);
		$('#slide_description').text(gallery_obj[gallery_slides_counter].slide_description);
		$('#photographer_name').text(gallery_obj[gallery_slides_counter].photographer_name);
	    $('#photographer_gps').text(gallery_obj[gallery_slides_counter].photographer_gps);
		
		slide_pos_counter -=1;
		document.getElementById("curr_slide_pos").innerHTML = slide_pos_counter;
  }

}

function nextSlide() {
  if(gallery_slides_counter == (total_slides-1)) {
	 ...
	 document.getElementById("curr_slide_pos").innerHTML = slide_pos_counter;
		...
		$("#next_nav_btn").click(function(){
			nextSlide();	
		});
		
		$("#prev_nav_btn").click(function(){
			prevSlide();
		});
		...
		$("#main-img, #zoomIn_btn").click(function(){
		 $("body").addClass("zoomed");
		 $("#main-img").addClass("zoomed-obj");
...
<script>
  $(function() {
	...
	<title><?php echo $post[0]->Post_Headline; ?> | NOMADMAG.co</title>
    ...
    <meta name="title" content="<?php echo $post[0]->Post_Headline; ?> | NOMADMAG.co" />
	...
	<meta property="og:site_name" content="NOMADMAG" />
	<meta property="og:title" content="<?php echo $post[0]->Post_Headline; ?> | NOMADMAG.co" />
   ...
   <style>
 	...
 	.photoRailContainer>.header>.genInfo>h3 {
	...
	.panel_bottom_data .photographer_info h3 {
		font-family: "museo-slab",serif;
		font-weight: 100;
		font-size: 14px;
		line-height: 22px;
...
.photoCarousel>.photoItems>.container{ text-align:center; }   
...
button.zoomOut{ background: url(<?php echo base_url();?>assets/images/zoomclose.png) no-repeat; display:none;}
button.zoomOut:hover{background: url(<?php echo base_url();?>assets/images/zoomclose.png) no-repeat;}



body.zoomed .photoCarousel{ width:100% !important;}
body.zoomed .photoCarousel>.photoItems>.container{overflow:scroll;}
body.zoomed .photoCarousel>.photoItems>.container img#main-img {
cursor: grab;
cursor: -moz-grab;
cursor: -webkit-grab;
height: auto;
...
body.zoomed button.zoomOut {
background-image: .pxCloseLight;
...
body.zoomed button.zoomIn{ display:none;}
...
<body class="imageGalleryTemplate">
...
<div id="photoDetailContainer">
    ...
    <img class="dragger" src="<?php echo base_url();?>/uploads/gallery/image-gallery/<?php echo $gallerydata[0]["Slide_Src_File"]; ?>" alt="" id="main-img" />
    <button class="zoomOut pxCloseDark" id="zoomclose_btn">close zoom</button>
    <button id="zoomIn_btn" class="zoomIn">zoom in</button>
    <ul style="display:none">
    <?php foreach( $gallerydata as $galleryslide ) : ?>
      <li><img src="<?php echo base_url();?>/uploads/gallery/image-gallery/<?php echo $galleryslide["Slide_Src_File"]; ?>" alt="" /></li>
   <?php endforeach; ?>
    </ul>
...
<aside class="photoRailContainer">
    ...
    <button id="prev_nav_btn" class="prevArrow prevItem">prev</button>
    <button id="next_nav_btn" class="nextArrow nextItem">next</button>
    <p class="gallery_slide_counter"><span class="position" id="curr_slide_pos">1</span><span class="slash">/</span><?php echo count($gallerydata);?></p>
    </div>
    
    
	<section class="header">
		<div class="genInfo ">
                <h3><?php echo $post[0]->Post_Author; ?>:</h3>
		    <h2><span id="slide_title"><?php echo $gallerydata[0]["Slide_Title"]; ?></span></h2>
		</div>	
	</section>
	
	<section class="header description">
		<div id="content_1" class="sidebar_scroller_content">
			<p id="slide_description"><?php echo $gallerydata[0]["Slide_Description"]; ?></p>
		</div>
            
        <div class="panel_bottom_data">
            <div class="photographer_info">
            <h10>Photography:
            	<h3><span id="photographer_name"><?php echo $gallerydata[0]["Photographer_Name"]; ?></span></h3>
                <span id="photographer_gps"><?php echo $gallerydata[0]["Photographer_Gps"]; ?></span>
            </h10>
            </div>
    	</div>
